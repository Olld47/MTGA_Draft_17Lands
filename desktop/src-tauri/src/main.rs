...
use mtga_bridge_lib::{ext_mod, tauri_generate_context};
...
fn main() -> Result<Infallible, Box<dyn Error>> {
    ...
    // 👉 Equivalent to `python -m mtga_bridge`,
    // i.e, run the `src-tauri/python/mtga_bridge/__main__.py`
    let py_script = PythonScript::Module("mtga_bridge".into());
