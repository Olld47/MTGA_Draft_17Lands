...
class CustomDeckPanel(ttk.Frame):
    ...
    # State is owned by the shared src.deck_actions.DeckActions layer; these
    # properties keep the existing read sites working unchanged (ticket 09).
    @property
    ...
    def refresh(self):
        ...
        raw_pool = self.draft.retrieve_taken_cards()
        if not raw_pool:
            self.actions.refresh_pool(raw_pool)
            self._update_tables()
            self._update_basics_toolbar()
            return

        if len(raw_pool) > self.actions.known_pool_size:
            self.actions.refresh_pool(raw_pool)
            self._update_tables()
            self._render_deck_stats()
    ...
    def _run_simulation(self):
        ...
        self.sim_executor.submit(self._run_monte_carlo_task)

    def _run_monte_carlo_task(self):
        ...
        try:
            ...
            if ok:
                ...
                self.after(0, lambda: self._show_sim_error(message))
    ...
    def _run_auto_optimize_task(self):
        ...
        try:
            ...
            if ok:
                ...
                def finalize():
                    self._update_tables()
                    self._show_sim_results(stats, optimization_note=note)
                    self._render_deck_stats()
                    self._draw_sample_hand()
                    self._update_basics_toolbar()
                ...
                def show_err():
                    self._show_sim_error(message)
                    import tkinter.messagebox

                    tkinter.messagebox.showwarning(
                        "Optimization Failed", message, parent=self
                    )
    ...
    def _move_card(self, source_list, dest_list, card_name):
        self.actions.move_card(card_name, to_sideboard=dest_list is self.sb_list)
        self._update_tables()
        self._render_deck_stats()
        self._update_basics_toolbar()
    ...
    def _run_auto_lands_task(self):
        ...
        try:
            _, message, stats, _ = self.actions.apply_auto_lands()
    ...
    def _copy_to_clipboard(self):
        ...
        self.clipboard_append(self.actions.export_text())
