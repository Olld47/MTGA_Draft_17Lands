...
def test_greedy_double_pip_bomb_splash(mock_metrics):
    ...
    # Base WB Pool
    pool = [
        {"name": f"White 2-Drop {i}", "types": ["Creature"], "colors": ["W"], "cmc": 2, "mana_cost": "{1}{W}", "deck_colors": {"All Decks": {"gihwr": 57.0}}}
        for i in range(10)
    ] + [
        {"name": f"Black 3-Drop {i}", "types": ["Creature"], "colors": ["B"], "cmc": 3, "mana_cost": "{2}{B}", "deck_colors": {"All Decks": {"gihwr": 57.0}}}
        for i in range(10)
    ]
    ...
    greedy_deck_nofix, splash_color_nofix = build_variant_greedy(pool, ["W", "B"], mock_metrics)
    ...
    pool.extend([
        {"name": "WB Dual", "types": ["Land"], "colors": ["W", "B"], "deck_colors": {"All Decks": {"gihwr": 54.0}}},
        {"name": "WU Dual", "types": ["Land"], "colors": ["W", "U"], "deck_colors": {"All Decks": {"gihwr": 54.0}}},
        {"name": "BU Dual", "types": ["Land"], "colors": ["B", "U"], "deck_colors": {"All Decks": {"gihwr": 54.0}}},
        {"name": "Treasure Dork", "types": ["Creature"], "colors": ["W"], "cmc": 2, "mana_cost": "{1}{W}", "tags": ["fixing_ramp"], "oracle_text": "Create a treasure", "deck_colors": {"All Decks": {"gihwr": 55.0}}}
    ])

    greedy_deck_fix, splash_color_fix = build_variant_greedy(pool, ["W", "B"], mock_metrics)
    
    assert splash_color_fix == "U", "Should actively embrace the 3UU bomb when fixing is heavy."
    ...
    assert stats["cast_t2"] > 40.0, "Core WB velocity destroyed by splash! Auto-Lands allocated basics poorly."
